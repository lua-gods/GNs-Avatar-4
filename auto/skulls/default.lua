...
local COLOR_SPECULAR = Color.parseColor("#ffeb78").rgb
...
identity.processHat = {
	ON_PROCESS = function(skull, model, delta)
		...
		if vars.color then
			...
			albedo = ok and value.xyz or COLOR_ALBEDO
		else
			albedo = COLOR_ALBEDO
		end
		model.cylinder:setScale(1, height, 1)
		model.top:setPos(0, height, 0)
		model.ribbon.shade1:setColor(math.lerp(albedo, COLOR_AMBIENT,0.5))
		model.ribbon.shade2:setColor(math.lerp(albedo, COLOR_AMBIENT,0.25))
		model.ribbon.shade3:setColor(albedo)
		model.ribbon.shade4:setColor(math.lerp(albedo, COLOR_SPECULAR,0.25))
		model.ribbon.shade5:setColor(math.lerp(albedo, COLOR_SPECULAR,0.5))
	end,
