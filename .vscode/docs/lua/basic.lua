...
---[§2.2](command:extension.lua.doc?["en-us/52/manual.html/2.2"])). Lua itself does not use this variable; changing its
...
---Raises an error if the value of its argument v is false (i.e., `nil` or `false`); otherwise, returns all its
---arguments. In case of error, `message` is the error object; when absent, it defaults to `"assertion failed!"`
---
...
---
---Usually, `error` adds some information about the error position at the beginning of the message, if the message is a
---string.
---
...
---in this table. A call to `next` returns the next index of the table and its associated value. When called with `nil`
---as its second argument, `next` returns an initial index and its associated value. When called with the last index, or
---with `nil` in an empty table, `next` returns `nil`. If the second argument is absent, then it is interpreted as
...
---table. You may however modify existing fields. In particular, you may set existing fields to nil.
---
...
---This function does not exist in normal Lua 5.2 and is somewhat similar to the Lua 5.1 function of the same name.
---@param text string
...
---Sets the metatable for the given table. If `metatable` is `nil`, removes the metatable of the given table. If the
---original metatable has a `__metatable` field, raises an error.
---
...
---Sets the real value of `table[index]` to `value`, without using the `__newindex` metamethod. `table` must be a table,
...
---Calls the function `f` with the given arguments in *protected mode*. This means that any error inside `f` is not
---propagated; instead, `pcall` catches the error and returns a status code. Its first result is the status code (a
---boolean), which is true if the call succeeds without errors. In such case, `pcall` also returns all results from the
---call, after this first result. In case of any error, `pcall` returns `false` plus the error object.
---
...
---represents 10, 'B' represents 11, and so forth, with 'Z' representing 35. If the string `e` is not a valid numeral in
...
---
---If a non-string value is given, the value will be formatted in a readable manner and given a special color depending
---on the type.
---* `nil`: Red,
---* `boolean`: Purple,
---* `number`: Cyan,
...
---| "ViewerAPI"
---| "WorldAPI"
--- <sck> If you know what this is and have access to it, remove the second | from the entries above to re-enable them.
