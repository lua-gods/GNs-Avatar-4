...
---@field hoverEvent GNUI.Box? # the element the mouse is currently hovering over
...
---@field PASSIVE_HOVERING_ELEMENT_CHANGED Event # Just like the HOVERING_BOX_CHANGED, but will never be nil if there is nothing being hovered.
---
---@field pressedElements GNUI.Box[]? # the last pressed element, used to unpress buttons that have been unhovered.
---@field MOUSE_MOVED_GLOBAL Event # called when the mouse position changes
...
---@field INPUT Event # serves as the handler for all inputs within the boundaries of the canvas. called with the first argument being an input event
---@field UNHANDLED_INPUT Event # triggers when an input is not handled by the children of the canvas.
local Canvas = {}
...
function Canvas:setMousePos(x, y)
	...
	if relative.x ~= 0 or relative.y ~= 0 then
		...
		if self.hoverEvent then self.hoverEvent.MOUSE_MOVED:invoke(event) end

		for _, e in pairs(self.pressedElements) do
			if e ~= self.hoverEvent then e.MOUSE_MOVED:invoke(event) end
		end
...
function parseInputEventToChildren(element, event, position)
	if element.parent then position = position - element.finalDimensions.xy end
...
function Canvas:parseInputEvent(key, state, shift, ctrl, alt, char, strength)
	...
	if not captured then
		...
		for _, e in pairs(self.pressedElements) do
			if e ~= self.hoverEvent and e.Canvas == self then
	...
	if key ~= "key.mouse.scroll" then
		if state ~= 0 then -- QOL feature that allows boxes to recive a button being unpressed even when not hovered anymore.
			self.pressedElements[key] = self.hoverEvent
		else
...
function Canvas:getChildFromPos(pos)
	if self.parent then pos = pos - self.finalDimensions.xy end
...
function Canvas:setHoveringChild(box)
	if box ~= self.hoverEvent then
		if self.hoverEvent then
			self.hoverEvent:setIsCursorHovering(false)
...
---@return GNUI.Box
function Canvas:getHoveredElement() return self.hoverEvent end
